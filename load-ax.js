...
const rows = data.map((d) => {
  ...
  const description = argv.axType == "pending" ? (d[8] ? d[8] : "") : d[7] ? d[7] : ""
    ...
    objList =
      foundJiraCodes && foundJiraCodes.length > 0
        ? foundJiraCodes.map((o) => {
            ...
            return key && key.length > 0 && hoursToDot != "" ? { key: key[0].trim().toUpperCase(), hours: hoursToDot } : false
          })
        : []
  }
        ...
        argv.axType == "pending"
          ? {
              cells: [
                ...
                { column: "Activity number", value: d[6] },
                { column: "Activity", value: d[7] },
                { column: "Description", value: `SUBCALCULATION on ${o.key} = ${o.hours ? o.hours : 0}h.\nPARENT description: ${d[8]}` },
                { column: "Resource name", value: d[13] },
                { column: "Invoice status", value: "" },
                { column: "Transaction type", value: d[18] },
...
const updatecoda = async () => {
  ...
  console.log("1 ROW flat:", JSON.stringify(rows[0], null, 2))

  // Copy/add data to Coda Table
